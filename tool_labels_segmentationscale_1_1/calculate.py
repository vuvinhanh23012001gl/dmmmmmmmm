...
def auto_segment(image, model, confidence, polygons_data_list, img_mask, yolo_bboxes):
    ...
    # 👉 cập nhật polygons_data_list ngay
    contours, _ = cv2.findContours(img_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for contour in contours:
        poly = [tuple(pt[0]) for pt in contour]
        polygons_data_list.append({"label": "none", "polygon": poly})




def save_current_state(show_bbox,file_path, current_image):
