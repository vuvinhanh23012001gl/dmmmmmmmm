...
path_weight_giac = r"D:\tupn\yolo\runs\detect\fuser_dll_3_label\weights\best.pt"
load_model = 0
model = ""    
...
def crop_images_with_detection_from_folder(input_folder_path, output_folder_path, delta=20):
    ...
    for image_filename in os.listdir(input_folder_path):
        ...
        xyxy_list, labels, _, error_msg = xu_ly_anh(img)
        ...
        for i, box_coords in enumerate(xyxy_list):
            ...
            output_folder_path_new = output_folder_path + "/" + labels[i]
            if not os.path.exists(output_folder_path_new):
                os.makedirs(output_folder_path_new)
            output_path = os.path.join(output_folder_path_new, cropped_filename)
...
if __name__ == '__main__':
    path_input_folder = r"D:\tupn\data_training\fuser_dll_a_Thai_Cuong_new\anh_them\images"
    path_output_folder = r"D:\tupn\data_training\fuser_dll_a_Thai_Cuong_new\anh_them\convert_3_label"
